...
// ValidateEmail validates an email address (legacy function, uses RFC-compliant validation)
...
func ValidateEmailRFC(email string) error {
	...
	// Optional: DNS MX record validation (disabled by default for performance)
	// To enable DNS validation, call ValidateEmailWithDNS instead

	return nil
}

// ValidateEmailWithDNS validates an email address and checks if the domain has valid MX records
func ValidateEmailWithDNS(email string) error {
	// First do RFC validation
	...
	if err != nil || len(mxRecords) == 0 {
		return fmt.Errorf("invalid email domain: no MX records found for %s", domain)
	}
