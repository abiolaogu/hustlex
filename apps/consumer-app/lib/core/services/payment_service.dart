...
import 'paystack_stub.dart'; // Stub for flutter_paystack - TODO: replace with actual SDK
  ...
  Future<PaymentResult> processCardPayment({
    ...
    try {
      ...
      if (response.status) {
        ...
        final message = response.message ?? 'Card payment failed';
  ...
  Future<PaymentResult> processBankTransfer({
    ...
    try {
      ...
      if (response.status) {
        ...
        final message = response.message ?? 'Bank transfer failed';
        if (message.contains('cancelled')) {
          return PaymentResult.cancelled();
        }
        return PaymentResult.failure(message);
